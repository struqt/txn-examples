...
declare -r DB_TYPE='postgres'
declare -r DB_HOST_PORT="${DB_HOST_PORT:-localhost:5432}"
declare -r DB_USER_PASS="${DB_USER_PASS:-example:abcDe123}"

declare -r REV="example_rev"
declare -r MIG="file://${MIG_DIR:?}"
declare -r DEV="${DB_TYPE:?}://${DB_USER_PASS:?}@${DB_HOST_PORT:?}/atlas_dev?sslmode=disable"
declare -r URL="${DB_TYPE:?}://${DB_USER_PASS:?}@${DB_HOST_PORT:?}/example?sslmode=disable"
