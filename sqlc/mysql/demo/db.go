...
func Prepare(ctx context.Context, db DBTX) (*Queries, error) {
	...
	if q.statAuthorStmt, err = db.PrepareContext(ctx, statAuthor); err != nil {
		return nil, fmt.Errorf("error preparing query StatAuthor: %w", err)
	}
	return &q, nil
}
...
func (q *Queries) Close() error {
	...
	if q.statAuthorStmt != nil {
		if cerr := q.statAuthorStmt.Close(); cerr != nil {
			err = fmt.Errorf("error closing statAuthorStmt: %w", cerr)
		}
	}
	return err
}
...
func (q *Queries) WithTx(tx *sql.Tx) *Queries {
	return &Queries{
		...
		statAuthorStmt:   q.statAuthorStmt,
	}
}
